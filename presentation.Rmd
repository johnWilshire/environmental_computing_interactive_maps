...
```{r, echo = F}
leaflet(width = 250, height = 250) %>% addTiles()
```
...
- We are going to save this map as a base layer for using later
...
obs <- read.csv('MyEBirdData.csv') %>% group_by(Latitude, Longitude) %>% 
  summarise(total_species = n_distinct(Common.Name),
            my_label = paste(total_species, 'species seen')) %>% ungroup()
...
base %>% 
  ...
  addPolylines(151.22 + 0.01 * runif(10), -33.93 + 0.01 * runif(10), color = 'purple') %>% 
